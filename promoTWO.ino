...
void setup() {
  ...
  if (SD.begin())
     ...
     sdcard_file.println("This data is for test@setup"); //Writing to file
...
void wrt(String stg){
      ...
      Serial.println(sdcard_file);
       if (sdcard_file) { //If the file is found
       Serial.println("Writing key 4 AF/D");
       sdcard_file.println(stg); //Writing to file
       sdcard_file.close(); //Closing the file
       }
  }
void wrtT(){
...
boolean stMa=0;
byte strH=0,stpH=0,strM=0,stpM=0,GtH=0,GtM=0;  
void sub(String subj,boolean b){
          tmElements_t tm;
          RTC.read(tm);
          Serial.print(String(subj)+" from tm time:");
          Serial.println(tm.Hour);
          Serial.print(String(subj)+ "from tm min:");
      ...
      Serial.println(sdcard_file);
       if (sdcard_file) { //If the file is found
        if(stMa==0){
          oled.clear();
          oled.println(String(subj)+" is  Running @"+String(time2string()));
           strH=tm.Hour;
           strM=tm.Minute;
           Serial.println(time2string());
           sdcard_file.println("===========================");
           sdcard_file.print(String(subj)+" started---> ");
           sdcard_file.println(time2string()); //Writing to file
           sdcard_file.close(); //Closing the file
           stMa=1;
        }else if(stMa==1){
          //oled.clear();
          oled.println(String(subj)+ "Stopped @"+String(time2string()));
          stpH=tm.Hour;
          stpM=tm.Minute;
           Serial.println(time2string());
